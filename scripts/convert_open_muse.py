...
from diffusers.pipelines.open_muse.pipeline_open_muse import OpenMusePipeline
from diffusers.schedulers import OpenMuseScheduler
...
def main():
    ...
    scheduler = OpenMuseScheduler(mask_token_id=old_transformer.config.mask_token_id)

    new_pipe = OpenMusePipeline(
...
def make_transformer(old_transformer, model_256):
    ...
    input_ids = torch.randint(0, 10, (1, 1024), device=old_transformer.device)
    ...
    old_out = old_transformer(input_ids, encoder_hidden_states, cond_embeds, micro_conds)
...
def make_vqvae(old_vae):
    ...
    print("TEMP skipping vae decoder equivalence check")
    ...
    print("TEMP skipping full vae equivalence check")
