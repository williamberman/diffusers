...
class AmusedInpaintPipelineSlowTests(unittest.TestCase):
    def test_amused_256(self):
        pipe = AmusedInpaintPipeline.from_pretrained("openMUSE/diffusers-pipeline-256")  # TODO change
        ...
        expected_slice = np.array([0.0186, 0.0181, 0.0088, 0.0175, 0.0236, 0.0101, 0.0219, 0.0359, 0.0077])
        assert np.abs(image_slice - expected_slice).max() < 3e-3

    def test_amused_512(self):
        pipe = AmusedInpaintPipeline.from_pretrained("openMUSE/diffusers-pipeline")  # TODO change
        pipe.to(torch_device)
