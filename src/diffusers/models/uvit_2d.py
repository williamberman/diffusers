...
class UVit2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, input_ids, encoder_hidden_states, cond_embeds, micro_conds, cross_attention_kwargs=None):
...
class UVitBlock(nn.Module):
    ...
    def forward(self, x, cond_embeds, encoder_hidden_states, cross_attention_kwargs):
        if self.downsample is not None:
            x = self.downsample(x)

        for res_block, attention_block in zip(self.res_blocks, self.attention_blocks):
            x = res_block(x, cond_embeds)
