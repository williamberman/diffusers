...
from diffusers import OpenMuseImg2ImgPipeline, OpenMuseScheduler, UVit2DModel, VQModel
...
class OpenMuseImg2ImgPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_class = OpenMuseImg2ImgPipeline
...
@slow
@require_torch_gpu
class OpenMuseImg2ImgPipelineSlowTests(unittest.TestCase):
    def test_open_muse_256(self):
        pipe = OpenMuseImg2ImgPipeline.from_pretrained("openMUSE/diffusers-pipeline-256")  # TODO change
        ...
        expected_slice = np.array([0.1392, -0.1608, 0.0182, 0.3891, 0.1801, 0.2481, 0.3631, 0.2468, 0.2214])
        assert np.abs(image_slice - expected_slice).max() < 3e-3

    def test_open_muse_512(self):
        pipe = OpenMuseImg2ImgPipeline.from_pretrained("openMUSE/diffusers-pipeline")  # TODO change
        pipe.to(torch_device)
        ...
        expected_slice = np.array([0.2204, 0.3126, 0.2906, 0.1317, 0.2686, 0.3264, 0.2724, 0.3101, 0.3296])
        assert np.abs(image_slice - expected_slice).max() < 3e-3
