...
from diffusers import OpenMusePipeline, OpenMuseScheduler, UVit2DModel, VQModel
...
class OpenMusePipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_class = OpenMusePipeline
...
@slow
@require_torch_gpu
class OpenMusePipelineSlowTests(unittest.TestCase):
    def test_open_muse_256(self):
        pipe = OpenMusePipeline.from_pretrained("openMUSE/diffusers-pipeline-256")  # TODO change
    ...
    def test_open_muse_512(self):
        pipe = OpenMusePipeline.from_pretrained("openMUSE/diffusers-pipeline")  # TODO change
        pipe.to(torch_device)
