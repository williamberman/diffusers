...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import OpenMuseInpaintPipeline
        >>> from diffusers.utils import load_image

        >>> pipe = OpenMuseInpaintPipeline.from_pretrained(
...
class OpenMuseInpaintPipeline(DiffusionPipeline):
    ...
    # Copied from diffusers.pipelines.open_muse.pipeline_open_muse.OpenMusePipeline.enable_fused_rms_norm
    def enable_fused_rms_norm(self):
        r"""
        Enable fused rms norm from flash attention package.
        https://github.com/Dao-AILab/flash-attention/tree/main/csrc/layer_norm

        Requires installing from flash attention source:
        ...
        ```
        """
        self.set_use_fused_rms_norm(True)

    # Copied from diffusers.pipelines.open_muse.pipeline_open_muse.OpenMusePipeline.disable_fused_rms_norm
    def disable_fused_rms_norm(self):
        self.set_use_fused_rms_norm(False)

    # Copied from diffusers.pipelines.open_muse.pipeline_open_muse.OpenMusePipeline.set_use_fused_rms_norm
    def set_use_fused_rms_norm(self, use_fused_rms_norm: bool) -> None:
        def fn_recursive_set(module: torch.nn.Module):
            if hasattr(module, "set_use_fused_rms_norm"):
                module.set_use_fused_rms_norm(use_fused_rms_norm)

            for child in module.children():
                fn_recursive_set(child)
    ...
    def __call__(
        ...
        self.scheduler.set_timesteps(num_inference_steps, self._execution_device)
        ...
        latents = self.vqvae.quantize(latents)[2][2]
        latents = latents.reshape(latents_bsz, -1)
        ...
        latents = self.scheduler.add_noise(latents, self.scheduler.timesteps[start_timestep_idx], generator=generator, mask=mask)
