...
class SpatialTransformer(ModelMixin, ConfigMixin):
    ...
    When inputs is continuous: First, project the input (aka embedding) and reshape to b, t, d. Then apply standard
    transformer action. Finally, reshape to image.

    When input is discrete: First, input (classes of latent pixels) has positional embeddings applied, see
    `ImagePositionalEmbeddings`. Then apply standard transformer action. Finally, predict classes of unnoised image.
    ...
    Parameters:
        ...
        discrete (:
            ...
            vector embeddings for each pixel. See the docstring for a more in-depth description.
        ...
        diffusion_steps (:obj: `int`, *optional*): The number of diffusion steps used during training. Note that
            this is fixed at training time as it is used to learn a number of embeddings that are added to the hidden
            states. During inference, you can denoise for up to but not more than `diffusion_steps`.
        attention_bias (:
    ...
    def __init__(
        ...
        ff_layers: Optional[List[Literal["Dropout", "Linear", "ApproximateGELU", "GEGLU"]]] = None,
        norm_layers: Optional[List[Literal["LayerNorm", "AdaLayerNorm"]]] = None,
        ...
        if norm_layers is not None:
            transformer_kwargs["norm_layers"] = norm_layers
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        ff_layers: Optional[List[Literal["Dropout", "Linear", "ApproximateGELU", "GEGLU"]]] = None,
        norm_layers: List[Literal["LayerNorm", "AdaLayerNorm"]] = ["LayerNorm", "LayerNorm", "LayerNorm"],
        diffusion_steps: Optional[int] = None,
        attention_bias: Optional[bool] = None,
    ):
        super().__init__()
        ...
        if attention_bias is not None:
            attn_kwargs["bias"] = attention_bias

        self.attn1 = CrossAttention(
            query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout, **attn_kwargs
        )  # is a self-attention

        ff_kwargs = {}

        if ff_layers is not None:
            ff_kwargs["layers"] = ff_layers

        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff, **ff_kwargs)

        self.attn2 = CrossAttention(
            query_dim=dim, context_dim=context_dim, heads=n_heads, dim_head=d_head, dropout=dropout, **attn_kwargs
    ...
    def forward(self, hidden_states, context=None, timestep=None):
        ...
        if timestep is not None:
            norm_kwargs["timestep"] = timestep

        hidden_states = self.attn1(self.norm1(hidden_states, **norm_kwargs)) + hidden_states
        hidden_states = self.attn2(self.norm2(hidden_states, **norm_kwargs), context=context) + hidden_states
...
class FeedForward(nn.Module):
    ...
    Parameters:
        ...
        layers (:obj:,`List[Literal["Dropout", "Linear", "ApproximateGELU", "GEGLU"]]` *optional*):
            ...
            and GEGLU) but as many non-dimension changing layers as you want (Dropout and ApproximateGELU). The first
            dimension changing layer will project from the input dimension to the hidden dimension. The second
            dimension changing layer will project to the output dimension.
    """
    ...
    def __init__(
        ...
        glu: bool = False,
        dropout: float = 0.0,
        layers: List[Literal["Dropout", "Linear", "ApproximateGELU", "GEGLU"]] = ["GEGLU", "Dropout", "Linear"],
    ):
        super().__init__()
        ...
        dim_idx = 0
        dims = [[dim, inner_dim], [inner_dim, dim]]
