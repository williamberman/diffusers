...
class VQDiffusionSchedulerOutput(BaseOutput):
    ...
    Args:
        prev_sample (`torch.LongTensor` of shape `(batch size, num latent pixels)`):
            Computed sample x_{t-1} of previous timestep. `prev_sample` should be used as next
            model input in the denoising loop.
    """
    prev_sample: torch.LongTensor

def index_to_log_onehot(x: torch.LongTensor, num_classes: int) -> torch.FloatTensor:
    """
    Convert batch of vector of class indices into batch of vectors of log onehot vectors
...
def alpha_schedules(num_diffusion_timesteps: int, a_cumulative_start = 0.99999, a_cumulative_end = 0.000009):
    ...
    See section 4.1.
    """
    att = np.arange(0, num_diffusion_timesteps)/(num_diffusion_timesteps-1)*(a_cumulative_end - a_cumulative_start) + a_cumulative_start
...
def gamma_schedules(num_diffusion_timesteps: int, c_cumulative_start = 0.000009, c_cumulative_end = 0.99999):
    ...
    See section 4.1.
    """
    ctt = np.arange(0, num_diffusion_timesteps)/(num_diffusion_timesteps-1)*(c_cumulative_end - c_cumulative_start) + c_cumulative_start
...
class VQDiffusionScheduler(SchedulerMixin, ConfigMixin):
    ...
    The VQ-diffusion scheduler converts the transformer's output into a sample for the unnoised
    image at the previous diffusion timestep.
    ...
    Args:
        num_embed (`int`):
            The number of classes for the vector embeddings of the latent pixels. Includes the
            class for the masked latent pixel.
    ...
    def __init__(
        ...
        at, att = alpha_schedules(num_train_timesteps, a_cumulative_start=a_cumulative_start, a_cumulative_end=a_cumulative_end)
        ct, ctt = gamma_schedules(num_train_timesteps, c_cumulative_start=c_cumulative_start, c_cumulative_end=c_cumulative_end)
