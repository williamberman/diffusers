...
class CrossFrameAttnProcessor:
    ...
    def __init__(self, batch_size=2, enabled=True):
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if not is_cross_attention and self.enabled:
...
class OpenMuseTextToVideoZeroPipeline(StableDiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        # )
        processor = (
            CrossFrameAttnProcessor(batch_size=2)
        )
    ...
    def backward_loop(
        ...
        with self.progress_bar(total=num_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if _second_bward_loop and (
                    (not (noise_pred[0] == noise_pred[1]).all()) or 
                    (not (noise_pred[2] == noise_pred[3]).all())
                ):
                    ...
                    # assert False

                # if _second_bward_loop:
                #     print(noise_pred[0].float().abs().sum())
                #     print(noise_pred[1].float().abs().sum())
    ...
    def __call__(
        ...
        muse_scheduler_copy = copy.deepcopy(self.muse_pipe.scheduler)
        ...
        # muse_x_1_t0 = self.backward_loop(
        #     muse=True,
        #     timesteps=muse_timesteps[-muse_t1 - 1 : -muse_t0 - 1],
        #     prompt_embeds=None,
        #     muse_prompt_embeds=muse_prompt_embeds,
        #     muse_micro_conds=muse_micro_conds,
        #     muse_encoder_hidden_states=muse_encoder_hidden_states,
        ...
        # assert muse_t0 == muse_t1
        muse_x_2k_t1 = muse_x_2k_t0

        # muse_x_2k_t1 = self.muse_pipe.scheduler.add_noise(muse_x_2k_t0, muse_t1, generator=muse_generator)
        ...
        # muse_generator = [torch.Generator('cuda').manual_seed(0), torch.Generator('cuda').manual_seed(0)]

        global _second_bward_loop
        _second_bward_loop = True
        ...
        self.muse_pipe.scheduler = muse_scheduler_copy
        muse_x_1k_0 = self.backward_loop(
            muse=True,
            timesteps=muse_t0,
            # timesteps=muse_timesteps[-muse_t1 - 1 :],
            # timesteps=muse_timesteps[-muse_t1 - 3 :],
            # timesteps=muse_timesteps[-muse_t1 - 7 :],
            prompt_embeds=None,
            muse_prompt_embeds=muse_prompt_embeds,
            muse_micro_conds=muse_micro_conds,
            muse_encoder_hidden_states=muse_encoder_hidden_states,
            muse_generator=muse_generator,
            latents=muse_x_1k_t1,
            guidance_scale=muse_guidance_scale,
            callback=callback,
            callback_steps=callback_steps,
            extra_step_kwargs=extra_step_kwargs,
